...
namespace AlanJuden.MvcReportViewer.NetCore2.Models
{
	public class ReportViewerModel
	{
		...
		private static readonly List<string> KEYS_TO_IGNORE = new List<string>() { "ReportViewerEnablePaging", "reportPath", "startPage", "searchText", "page", "format" };
		private static readonly string[] VALUE_SEPARATORS = new string[] { "," };
		...
		public bool IsMissingAnyRequiredParameterValues(List<ReportParameterInfo> parameters)
		{
			...
			var missingValueParameters = matchedParameters.Where(x => x.Value == null || x.Value.Length == 0 || (x.Value.Where(string.IsNullOrEmpty)).Any());
